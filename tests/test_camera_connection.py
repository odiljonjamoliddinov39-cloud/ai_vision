...
def test_hikvision_worker_falls_back_to_substream_after_main_profile_failures():
    from streams.manager import _rtsp_source_for_attempt

    source = "rtsp://admin:secret@87.192.242.82:554/Streaming/Channels/1101"

    assert _rtsp_source_for_attempt(source, 0) == source
    assert _rtsp_source_for_attempt(source, 1) == source
    assert _rtsp_source_for_attempt(source, 2).endswith("/Streaming/Channels/1102")
