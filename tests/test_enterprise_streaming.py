...
def test_mediamtx_registration_uses_required_path_recording_variable(monkeypatch):
    ...
    client.ensure_source("274", "rtsp://camera.example.test/live")

    assert requests[0][0] == "POST"
    assert requests[0][2]["recordPath"] == (
        "/recordings/%path/%Y/%m/%d/%H-%M-%S-%f"
    )


def test_enterprise_ffmpeg_decodes_once_and_publishes_specialized_feeds(monkeypatch):
    monkeypatch.setenv("PUBLISH_MEDIAMTX_RENDITIONS", "true")
    client = FakeMediaMTX()
...
def test_default_mediamtx_worker_only_decodes_dashboard_websocket_preview(monkeypatch):
    monkeypatch.delenv("PUBLISH_MEDIAMTX_RENDITIONS", raising=False)
    client = FakeMediaMTX()
    route = client.route("7")
    command = _ffmpeg_command(route.source_rtsp_url, media_route=route)

    assert route.ai_rtsp_url not in command
    assert route.dashboard_rtsp_url not in command
    assert "split=3" not in " ".join(command)
    assert command.count("-i") == 1
