...
const LIVE_FRAME_DIAGNOSTIC_INTERVAL_MS = 7500;
...
let liveWsBytes = 0;
let liveWsLastBytes = 0;
let liveWsLastRenderedFrames = 0;
...
function renderLiveBitmap(slot, bitmap, metadata = null) {
  ...
  canvases.forEach((canvas) => {
    ...
    const latency = metadata?.generatedAtMs
      ? Math.max(0, Date.now() - backendClockOffsetMs - metadata.generatedAtMs)
      : null;
    canvas.parentElement?.querySelector("[data-live-latency]")?.replaceChildren(
      document.createTextNode(latency == null ? "—" : `${latency} ms`)
    );
...
const liveFramePipeline = new LiveFramePipeline({
  decodeFrame: (jpegBytes) => createImageBitmap(new Blob([jpegBytes], { type: "image/jpeg" })),
  renderFrame: renderLiveBitmap,
});
if (liveFrameDebugEnabled) {
  window.aiVisionLiveDiagnostics = () => liveFramePipeline.diagnostics(
    liveFrameSocket?.readyState ?? WebSocket.CLOSED
  );
}

function reportLiveFrameDiagnostics() {
  if (!liveFrameDebugEnabled) return;
  const now = Date.now();
  if (now - liveFrameLastDiagnosticAt < LIVE_FRAME_DIAGNOSTIC_INTERVAL_MS) return;
  liveFrameLastDiagnosticAt = now;
  console.debug("[live-frame-diagnostics]", {
    webSocketSlots: liveFrameSocketSlots,
    webSocketReadyState: liveFrameSocket?.readyState ?? WebSocket.CLOSED,
    cameras: liveFramePipeline.diagnostics(liveFrameSocket?.readyState ?? WebSocket.CLOSED),
  });
}

function enqueueLiveFrame(slot, jpegBytes, cacheFrame = true, generatedAtMs = null) {
  if (cacheFrame) liveWsBytes += jpegBytes.byteLength;
  if (cacheFrame) liveFrameCache.set(slot, { jpegBytes, generatedAtMs });
  liveFramePipeline.enqueue(slot, jpegBytes, {
    countReceived: cacheFrame,
    metadata: { generatedAtMs },
  });
  reportLiveFrameDiagnostics();
}

function decodeNextLiveFrame(slot) {
  liveFramePipeline.decodeNext(slot);
}

function resetLiveFrameState(slot) {
  liveFramePipeline.reset(slot);
}
...
function reconcileLiveStreams() {
  ...
  slotNumbers.forEach((slot) => {
    ...
    if (cachedFrame) {
      enqueueLiveFrame(slot, cachedFrame.jpegBytes, false, cachedFrame.generatedAtMs);
    }
  });
  if (
    liveFrameSocket
  ...
  socket.addEventListener("message", (event) => {
    ...
    const generatedAtMs = Number(
      new DataView(payload.buffer, payload.byteOffset + 2, 8).getBigUint64(0)
    );
    enqueueLiveFrame(slot, payload.slice(10), true, generatedAtMs);
  });
  socket.addEventListener("close", () => {
    if (liveFrameSocket !== socket) return;
    liveFramePipeline.resetAll();
...
function waitForIceGathering(peer) {
  ...
  return new Promise((resolve) => {
    ...
    peer.addEventListener("icegatheringstatechange", function listener() {
...
async function startWebRtcVideo(video) {
  const slot = Number(video.dataset.liveSlot);
  if (!slot || livePeerConnections.has(slot)) return;
  const peer = new RTCPeerConnection();
  livePeerConnections.set(slot, peer);
  peer.addTransceiver("video", { direction: "recvonly" });
  peer.addEventListener("track", (event) => {
    video.srcObject = event.streams[0] || new MediaStream([event.track]);
    video.play().catch(() => {});
    const measureFrame = (_now, metadata) => {
      setFeedBadgeLive(video, true);
      const captureTime = Number(metadata?.captureTime);
      if (Number.isFinite(captureTime)) {
        video.dataset.displayLatencyMs = String(
    ...
    if (video.requestVideoFrameCallback) video.requestVideoFrameCallback(measureFrame);
  });
  peer.addEventListener("connectionstatechange", () => {
    if (["failed", "disconnected", "closed"].includes(peer.connectionState)) {
      setFeedBadgeLive(video, false);
    }
  });
  await peer.setLocalDescription(await peer.createOffer());
  await waitForIceGathering(peer);
  const response = await fetch(`${API_BASE}/api/v2/streams/webrtc/${slot}/whep`, {
    method: "POST",
...
async function reportWebRtcStats() {
  ...
  let jitterCount = 0;
  const latencies = [];
  for (const [slot, peer] of livePeerConnections.entries()) {
    const reports = await peer.getStats();
    let inbound = null;
    reports.forEach((report) => {
      if (report.type === "inbound-rtp" && report.kind === "video") inbound = report;
    });
    const video = els.moduleContent.querySelector(`[data-live-webrtc][data-live-slot="${slot}"]`);
    const metric = video?.parentElement?.querySelector("[data-live-latency]");
    if (metric && inbound) {
      const jitterMs = Math.round(Number(inbound.jitter || 0) * 1000);
      const fps = Math.round(Number(inbound.framesPerSecond || 0) * 10) / 10;
      totalFps += fps;
      totalBytes += Number(inbound.bytesReceived || 0);
      totalJitter += jitterMs;
      jitterCount += 1;
      const displayLatency = Number(video.dataset.displayLatencyMs);
      if (Number.isFinite(displayLatency)) latencies.push(displayLatency);
      metric.textContent = Number.isFinite(displayLatency)
        ? `${displayLatency} ms · ${fps} FPS · jitter ${jitterMs} ms`
        : `${fps} FPS · jitter ${jitterMs} ms`;
    }
  }
  const previousBytes = liveWebRtcSnapshot.totalBytes || totalBytes;
  liveWebRtcSnapshot = {
    fps: totalFps,
    bandwidthKbps: Math.max(0, (totalBytes - previousBytes) * 8 / 1000),
    latencyMs: latencies.length ? Math.round(latencies.reduce((a, b) => a + b, 0) / latencies.length) : null,
    jitterMs: jitterCount ? Math.round(totalJitter / jitterCount) : 0,
    totalBytes,
  };
}
...
async function sampleLiveComparison() {
  if (!els.moduleContent.querySelector("[data-live-comparison]")) return;
  const requestStartedAt = Date.now();
  const backend = await api("/api/v2/streams/runtime-metrics").catch(() => ({}));
  const requestFinishedAt = Date.now();
  if (Number.isFinite(Number(backend.measured_at_ms))) {
    backendClockOffsetMs = (
      (requestStartedAt + requestFinishedAt) / 2 - Number(backend.measured_at_ms)
    );
  }
  let sample;
  if (liveTransportMode === "websocket") {
    const diagnostics = liveFramePipeline.diagnostics(liveFrameSocket?.readyState ?? WebSocket.CLOSED);
    const latencies = diagnostics.map((item) => item.latestLatencyMs).filter(Number.isFinite);
    const renderedFrames = diagnostics.reduce(
      (sum, item) => sum + Number(item.renderedFrameCount || 0), 0
    );
    sample = {
      latency: latencies.length ? latencies.reduce((a, b) => a + b, 0) / latencies.length : null,
      fps: Math.max(0, renderedFrames - liveWsLastRenderedFrames),
      bandwidth: Math.max(0, (liveWsBytes - liveWsLastBytes) * 8 / 1000),
      jitter: null,
    };
    liveWsLastBytes = liveWsBytes;
    liveWsLastRenderedFrames = renderedFrames;
  } else {
    sample = {
      latency: liveWebRtcSnapshot.latencyMs,
      fps: liveWebRtcSnapshot.fps,
      bandwidth: liveWebRtcSnapshot.bandwidthKbps,
      jitter: liveWebRtcSnapshot.jitterMs,
    };
  }
  sample.cpu = backend.process_cpu_percent;
  sample.memory = backend.process_memory_mb;
  const samples = liveMetricSamples[liveTransportMode];
  samples.push(sample);
  if (samples.length > 60) samples.shift();
  const summary = els.moduleContent.querySelector("[data-live-comparison]");
  const rows = ["websocket", "webrtc"].map((mode) => {
    const values = liveMetricSamples[mode];
    const latency = averageMetric(values, "latency");
    const bandwidth = averageMetric(values, "bandwidth");
    const cpu = averageMetric(values, "cpu");
    const memory = averageMetric(values, "memory");
    const jitter = averageMetric(values, "jitter");
    return `<tr><td>${mode === "websocket" ? "A · JPEG/WS" : "B · WebRTC"}</td><td>${latency === null ? "Unavailable" : `${latency} ms`}</td><td>${bandwidth === null ? "—" : `${bandwidth} kbps`}</td><td>${cpu === null ? "—" : `${cpu}%`}</td><td>${memory === null ? "—" : `${memory} MB`}</td><td>${jitter === null ? "—" : `${jitter} ms`}</td><td>${values.length}s</td></tr>`;
  }).join("");
  summary.innerHTML = `<table class="detected-table"><thead><tr><th>Mode</th><th>Display latency</th><th>Bandwidth</th><th>Host CPU</th><th>Pipeline memory</th><th>Jitter</th><th>Measured</th></tr></thead><tbody>${rows}</tbody></table>`;
}

function stopWebRtcRefresh() {
  if (liveWebRtcStatsTimer !== null) {
    window.clearInterval(liveWebRtcStatsTimer);
    liveWebRtcStatsTimer = null;
  }
  livePeerConnections.forEach((peer) => peer.close());
  livePeerConnections.clear();
  els.moduleContent.querySelectorAll("[data-live-webrtc]").forEach((video) => {
...
function applyLiveTransportMode() {
  ...
  const videos = Array.from(els.moduleContent.querySelectorAll("[data-live-webrtc]"));
  videos.forEach((video) => {
    video.style.display = useWebRtc ? "block" : "none";
  });
  if (useWebRtc) {
    stopLiveFrameRefresh();
    videos.forEach((video) => {
      delete video.dataset.displayLatencyMs;
      setFeedBadgeLive(video, false);
      const metric = video.parentElement?.querySelector("[data-live-latency]");
      if (metric) metric.textContent = "Connecting WebRTC...";
    });
    videos.forEach((video) => startWebRtcVideo(video).catch((error) => {
      setFeedBadgeLive(video, false);
      video.parentElement?.querySelector("[data-live-latency]")?.replaceChildren(
        document.createTextNode(`WebRTC error: ${error.message}`)
      );
    }));
    if (liveWebRtcStatsTimer === null) {
      liveWebRtcStatsTimer = window.setInterval(reportWebRtcStats, 1000);
    }
  } else {
    stopWebRtcRefresh();
...
function bindLiveTransportControls() {
  els.moduleContent.querySelectorAll("[data-live-mode]").forEach((button) => {
    button.addEventListener("click", () => {
      liveTransportMode = button.dataset.liveMode;
      localStorage.setItem("ai_vision_live_transport", liveTransportMode);
...
function renderAccountModule() {
  ...
  if (menu.id === "feed") {
    ...
    els.moduleContent.innerHTML = `
      <section class="acc-block" style="padding:12px 16px;margin-bottom:14px">
